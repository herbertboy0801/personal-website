...
var Fixture = class {
  ...
  withHeadersFile({
    ...
    const dest = (0, import_node_path2.join)(pathPrefix, "_headers");
    const contents = headers2.map(
      ({ headers: headersValues, path: headerPath }) => `${headerPath}${import_node_os.EOL}${headersValues.map((header) => `  ${header}`).join(import_node_os.EOL)}`
    ).join(import_node_os.EOL);
